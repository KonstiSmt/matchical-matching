...
* For **tables**, identify the **relevant content columns (1–3)** and output each row as: first kept column, then **one empty line**, then the remaining kept columns merged into **one line** with single spaces (e.g., `ContentA` + `\n\n` + `ContentB` \[+ space + `ContentC` if present]).
...
3. **Extract per structure**
   ...
   * **Tabular mode:**
     ...
     * **For each row**, produce a single string with this composition:

       ```
       [KeptColumn1 cleaned]

       [KeptColumn2 cleaned][ + space + KeptColumn3 cleaned]
       ```
       * If only one kept column is present in that row, output only that one line.
       * If the first kept column is empty in that row and later kept columns remain, output the remaining kept columns merged with single spaces.
...
* **Verbatim Preservation with Layout Normalization**
  ...
  * **Do not** rephrase, translate, summarize, reorder, shorten, or de-duplicate.
  ...
  * In **tabular mode**, the **only** introduced newlines are the **single blank-line separator** between the first kept column block and the merged trailing block.
...
* **Column Detection (Tabular Mode)**
  ...
  * **Fallbacks & ambiguity:**
    ...
    * **Two** or **three** content-like columns detected → keep *(A,B\[,C])* in left-to-right order, then format as `A`, blank line, then `B` \[+ space + `C`].
...
* **Tables:** identify **1–3 relevant content columns** (requirement, proof/evidence, optional quantifier like years). **Ignore ID/weighting**. Format each row as first kept column, blank line, then merged trailing kept columns (`B` \[+ space + `C`]).
...
* **No additions/omissions:** never infer, de-duplicate, or drop qualifiers; never merge separate rows/bullets.
