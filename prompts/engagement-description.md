...
- **MixedVersion**: A short engagement-focused paragraph followed by action bullets, with consultant-evidenced contributions prioritized and project text used only for clarification. Output must be valid HTML with proper entity encoding and predefined exclusions applied. Project-level scope is omitted unless tied to consultant actions.
- **ParagraphVersion**: A seamless narrative centered on consultant contributions, using project context only to clarify those contributions. Roles and skills are integrated naturally, output remains valid HTML with proper entity encoding, and predefined exclusions are applied. Project-level scope is omitted unless tied to consultant actions.
...
```json
{
  ...
  "MixedVersion": "<p>The engagement contribution focused on translating stakeholder needs into implementation-ready delivery artifacts.</p><ul><li>Facilitated requirements workshops</li><li>Documented process gaps for delivery planning</li><li>Prepared implementation-ready user stories</li></ul>",
...
7.  **Action-Oriented, Consultant-First Clean Versions (Bullet/Mixed/Paragraph)**
    ...
    *   **ParagraphVersion**: A cohesive narrative (can be first-person) using `<p>` tags only and centered on consultant contributions. Roles/skills/technologies integrated naturally. Consultant description is primary; project description only clarifies consultant actions. Project-only context not tied to consultant contribution is dropped. **No bold formatting.** No bullet points. Output uses valid HTML with entity encoding.
