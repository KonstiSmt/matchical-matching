...
1.  **Original Version Rules**
    ...
    *   **RationaleOriginalVersion**: Provide a brief plain-text explanation focusing only on formatting conversion decisions for the OriginalVersion (Markdown removal, table handling, allowed tags, entity encoding) and the applied exclusion list categories. Do not discuss clean versions here.
...
- **BulletVersion**: An impersonal, action-focused overview where consultant-evidenced contributions are primary and project text is used only for clarification. Roles and skills *contextualize* actions rather than being listed generically. Output must be valid HTML with proper entity encoding and predefined exclusions applied. Project-level scope is omitted unless tied to consultant actions.
- **MixedVersion**: A short engagement-focused paragraph followed by action bullets, with consultant-evidenced contributions prioritized and project text used only for clarification. The first sentence must not begin with `I` or `Ich`. Output must be valid HTML with proper entity encoding and predefined exclusions applied. Project-level scope is omitted unless tied to consultant actions.
- **ParagraphVersion**: A seamless first-person narrative centered on consultant contributions, using project context only to clarify those contributions. Roles and skills are integrated naturally, output remains valid HTML with proper entity encoding, and predefined exclusions are applied. Project-level scope is omitted unless tied to consultant actions. The first sentence must not begin with `I` or `Ich`.
...
```json
{
  ...
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, dropped project-only context not tied to consultant contributions, and reduced overlap with project-description narrative. Applied adaptive sparse fallback when needed, avoided invented facts, and followed formatting rules (no forbidden tags, proper HTML entities).",
