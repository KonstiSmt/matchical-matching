...
- **Enriched Alternatives (Clean Versions Only)**: For the BulletVersion, MixedVersion, and ParagraphVersion, treat the `### Consultant Description` as the primary evidence source for what to include. Use the `### General Project Description` only to clarify, disambiguate, or lightly contextualize consultant actions that are already evidenced. Refrain from adding anything that is not present or reasonably implied in the inputs, and still exclude the predefined set of sensitive/redundant items. Roles and skills should be woven into descriptions of actions, not listed generically. **Project-level scope, goals, or outcomes must be omitted unless explicitly tied to the consultant’s actions.** If consultant text contains project-only context not tied to consultant contribution, exclude that context from clean versions.
...
1.  **Assess the Input**
    ...
    *   **Important**:
        ...
        *   The **BulletVersion**, **MixedVersion**, and **ParagraphVersion** can incorporate relevant details from **both** the `### Consultant Description` and the `### General Project Description`, while still excluding the predefined set of items (Project Name/Title, Client Name, specific date labels, Industry, Functional Area – both label and value). For these clean versions, treat consultant text as the primary source and use project text only to clarify consultant actions. These versions must remain focused on the consultant’s actions and must omit project-level scope that is not explicitly tied to what the consultant did.
...
2.  **Extract Relevant Details**
    ...
    *   For clean versions, first extract consultant-evidenced contributions, then add project context only when it materially clarifies those contributions. Do not convert every project-side detail into engagement text.
    *   For clean versions, classify statements into:
        *   consultant-specific contribution statements, and
        *   project-only context statements.
        Keep the first category. Drop the second category unless directly required to understand a consultant-specific contribution.
...
3.  **Generate Four Versions of the Consultant Engagement**
    ...
    *   **BulletVersion**:
        ...
        *   Use consultant-evidenced actions from the `### Consultant Description` as the baseline. Add details from the `### General Project Description` only when they directly clarify those actions. Do not add anything not present in the inputs or subject to the predefined exclusions detailed in `Specifics 1.c`.
...
4.  **HTML Output Formatting**
    ...
    *   **HTML Escaping**: Escape only the characters required for safe HTML text nodes: `&` as `&amp;`, `<` as `&lt;`, and `>` as `&gt;`. Preserve normal UTF-8 apostrophes and quotation marks as literal characters. Do **not** convert apostrophes or quotation marks into HTML entities such as `&#39;`, `&apos;`, `&#34;`, or `&quot;` unless those entity sequences are already present in the source text and must be preserved literally.
...
1.  **Original Version Rules**
    ...
    *   **RationaleOriginalVersion**: Provide a brief plain-text explanation focusing only on formatting conversion decisions for the OriginalVersion (Markdown removal, table handling, allowed tags, minimal HTML escaping) and the applied exclusion list categories. Do not discuss clean versions here.

2.  **Bullet Version Rules**
    *   **Input Sources & Integration**: Use both the `### Consultant Description` and the `### General Project Description` with strict source priority:
        *   Primary source: consultant-evidenced actions from `### Consultant Description`.
        *   Secondary source: project text only to clarify/disambiguate consultant actions already evidenced.
        *   Do not lift standalone project narrative into engagement bullets.
        Focus on extracting **actions, responsibilities, and achievements**.
    *   **Engagement Focus**: Remove project-level scope, goals, or outcomes unless explicitly tied to the consultant’s actions.
    *   **Content Boundary**: Explicitly drop project-only context statements that cannot be tied to a consultant-specific contribution.
    ...
    *   **Selection over Exhaustive Conversion**: Do not map every granular input item to its own bullet. Prioritize high-signal consultant contributions and merge overlapping points.
    ...
    *   **Redundancy Control**: Avoid repeating full project objective/challenge/scope statements that belong to project descriptions unless needed to understand a specific consultant action.
    ...
    *   **HTML Restriction**:
        *   Use only `<ul><li>` for structure. **No bold formatting (`<strong>`) or other formatting.** Escape only `&`, `<`, and `>` inside `<li>` text. Preserve normal UTF-8 apostrophes and quotation marks as literal characters.
        *   Do not end `<li>` text with a period.
    ...
    *   **RationaleCleanVersions**: Provide one brief rationale shared across Bullet/Mixed/Paragraph describing that consultant text was treated as primary evidence, project text was used only to clarify consultant actions, project-level overlap was reduced, and skills/roles/technologies were integrated contextually when evidence existed. Note that no new facts were invented and that formatting rules were followed.
    *   **Sparse Fallback (Clean Versions)**:
        *   Prefer explicit consultant-action evidence.
        *   If explicit action evidence is weak, you may still produce minimal meaningful consultant-oriented output using grounded role/context/skill signals, as long as you do not fabricate tasks.
        *   Avoid generic filler like "used X skills" without contribution context.
        *   If meaningful non-fabricated clean output is still not feasible, return empty clean versions.

3.  **Mixed Version Rules**
    *   **Input Sources**: Combine relevant points from both inputs with source priority: consultant-evidenced contributions first, project text second for clarification only. Do not invent details or include items from the predefined exclusion list in `Specifics 1.c`.
    *   **Structure**:
    ...
    *   **Engagement Focus**:
        ...
        *   Drop project-only context statements not tied to consultant contribution.
        *   Avoid retelling project-description content when it does not add consultant-specific contribution context.
    *   **HTML Restriction**:
        *   Use `<p>` for the paragraph and `<ul><li>` for bullets only. **No bold formatting (`<strong>`) or other formatting.** Escape only `&`, `<`, and `>` in text content. Preserve normal UTF-8 apostrophes and quotation marks as literal characters.
        *   In the bullet list, do not end `<li>` text with a period.
...
4.  **Paragraph Version Rules**
    *   **Input Sources**: Combine relevant points from both inputs with source priority: consultant-evidenced contributions first, project text second for clarification only. Do not invent details or include items from the predefined exclusion list in `Specifics 1.c`.
    *   **Narrative Flow**:
        *   Use mandatory first-person perspective in the consultant language and rewrite to form a cohesive paragraph or two.
    ...
    *   **Engagement Focus**:
        *   Remove project-level scope, goals, or outcomes unless explicitly tied to the consultant’s actions.
        *   Drop project-only context statements not tied to consultant contribution.
        *   Prefer concise consultant-centric synthesis over full project storyline repetition.
    *   **Length Control**:
        *   Prefer **1–2 short paragraphs**. Keep the narrative concise even when the input is long.
    *   **HTML Restriction**:
        *   Use only non-empty `<p>` tags. **No bold formatting (`<strong>`) or other formatting.** Escape only `&`, `<`, and `>` inside `<p>` text. Preserve normal UTF-8 apostrophes and quotation marks as literal characters.
    *   **No Separate Tech Lists**:
...
- **OriginalVersion**: A faithful and **complete** reproduction of the consultant’s own words (`### Consultant Description` only), with a predefined set of sensitive/redundant items (Project Name/Title, Client Name, specific date labels, Industry, Functional Area – both label and value – and any resulting empty headers) removed (see `Specifics 1.c` for full details), **all Markdown formatting removed**, and structures like tables reformatted into clean, valid HTML (`<p>`, `<ul>`, `<li>`) with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) for text content. Normal UTF-8 apostrophes and quotation marks stay literal. All original information content not subject to exclusion **must** be present and readable.
- **BulletVersion**: An impersonal, action-focused overview where consultant-evidenced contributions are primary and project text is used only for clarification. Roles and skills *contextualize* actions rather than being listed generically. Output must be valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) and predefined exclusions applied. Normal UTF-8 apostrophes and quotation marks stay literal. Project-level scope is omitted unless tied to consultant actions.
- **MixedVersion**: A short engagement-focused paragraph followed by action bullets, with consultant-evidenced contributions prioritized and project text used only for clarification. The first sentence must not begin with `I` or `Ich`. Output must be valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) and predefined exclusions applied. Normal UTF-8 apostrophes and quotation marks stay literal. Project-level scope is omitted unless tied to consultant actions.
- **ParagraphVersion**: A seamless first-person narrative centered on consultant contributions, using project context only to clarify those contributions. Roles and skills are integrated naturally, output remains valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`), and predefined exclusions are applied. Normal UTF-8 apostrophes and quotation marks stay literal. Project-level scope is omitted unless tied to consultant actions. The first sentence must not begin with `I` or `Ich`.
...
```json
{
  "RationaleOriginalVersion": "Preserved the single German sentence exactly as provided in the Consultant Description. No content matched the predefined exclusion rules (e.g., for labels like 'Project Title:', 'Client:', 'Zeitraum:', 'Industry:', 'Functional Area:', or empty labels), so nothing was removed. No lists, tables, or Markdown formatting were present. Valid HTML using only <p> tags with minimal HTML escaping applied to content (`&amp;`, `&lt;`, and `&gt;` only), while normal UTF-8 apostrophes and quotation marks were preserved as literal characters. No forbidden formatting used. Language: German.",
  "OriginalVersion": "<p>Ich habe die interne IT-Hotline neu strukturiert und ein Ticket-System eingeführt.</p>",
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, dropped project-only context not tied to consultant contributions, and reduced overlap with project-description narrative. Applied adaptive sparse fallback when needed, avoided invented facts, and followed formatting rules (no forbidden tags, minimal HTML escaping only for `&amp;`, `&lt;`, and `&gt;`; normal UTF-8 apostrophes and quotation marks kept literal).",
...
```json
{
  "RationaleOriginalVersion": "Removed heading '## Projekt: Projektmanagement Schulungskonzeption' (due to 'Projekt:' label for Project Title). Removed '#### Zeitraum: 03/2012 - 12/2012 | 9 Monate' (due to 'Zeitraum:' label). Removed '### Unternehmen/Branche: KfW/Finanzwesen' (due to 'Unternehmen:' label for Client and 'Branche:' label for Industry). These are predefined excludable labels. Preserved '#### Projektbeschreibung:' and '#### Aufgaben:' headings as <p> tags (after removing heading markdown) as these labels are not on the exclusion list. Preserved the text under Projektbeschreibung. Converted the task under 'Aufgaben:' (starting with '-') into a list item <ul><li>. Ignored Markdown bold around 'IT Security'. Preserved '#### Teamgröße: 10' and '#### Technologien und Tools: MS Office' lines as <p> tags (after removing heading markdown) as these labels are not excluded and have content. Valid HTML using allowed tags with minimal HTML escaping applied (`&amp;`, `&lt;`, and `&gt;` only), while normal UTF-8 apostrophes and quotation marks were preserved as literal characters. No forbidden formatting used. Language: German.",
...
```json
{
  ...
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, dropped project-only context not tied to consultant contributions, and reduced overlap with project-description narrative. Applied adaptive sparse fallback when needed, avoided invented facts, and followed formatting rules (no forbidden tags, minimal HTML escaping only for `&amp;`, `&lt;`, and `&gt;`; normal UTF-8 apostrophes and quotation marks kept literal).",
  "BulletVersion": "<ul><li>Leadership of the technical workstream to improve tracking accuracy (&gt;15%)</li><li>Management of the technical team responsible for API integration and database optimization</li><li>Coordination of system deployment activities across involved teams</li><li>Application of system architecture and performance tuning principles to integration and database work</li></ul>",
...
```json
{
  ...
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, dropped project-only context not tied to consultant contributions, and reduced overlap with project-description narrative. Applied adaptive sparse fallback when needed, avoided invented facts, and followed formatting rules (no forbidden tags, minimal HTML escaping only for `&amp;`, `&lt;`, and `&gt;`; normal UTF-8 apostrophes and quotation marks kept literal).",
  "BulletVersion": "<ul><li>Definition of cloud strategy and leadership as Cloud Architect</li><li>Implementation of CI/CD pipelines as Lead DevOps Engineer</li><li>Application of Agile and Scrum methods to structure delivery of cloud and pipeline work</li></ul>",
  "MixedVersion": "<p>Cloud strategy definition and CI/CD implementation formed the core of my engagement within a team of 15 consultants.</p><ul><li>Definition of cloud strategy and leadership as Cloud Architect</li><li>Implementation of CI/CD pipelines as Lead DevOps Engineer</li><li>Application of Agile and Scrum methods to structure delivery of cloud and pipeline work</li></ul>",
  "ParagraphVersion": "<p>Cloud strategy definition and CI/CD implementation formed the core of my engagement as Cloud Architect and Lead DevOps Engineer. I led strategy decisions and delivered CI/CD pipelines within a team of 15 consultants.</p><p>I applied Agile and Scrum methods to structure delivery and keep the implementation aligned with requirements.</p>"
}
```
...
```json
{
  "RationaleOriginalVersion": "Preserved the consultant's English text, including both paragraphs. Markdown bold around 'continuous integration pipeline' was ignored (asterisks removed, text rendered normally). The 'Technologies used:' line and its list were fully preserved as a paragraph, as this label is not on the predefined exclusion list and the content does not match any excludable items. Valid HTML using only <p> tags with minimal HTML escaping applied (`&amp;`, `&lt;`, and `&gt;` only), while normal UTF-8 apostrophes and quotation marks were preserved as literal characters. No forbidden formatting used. Language: English.",
...
## Example 8 (Project-Only Context Dropped from Clean Versions)

### Input
```markdown
### Consultant Description
The project was initiated to standardize onboarding across five business units and to increase process transparency.
The target architecture introduced a centralized workflow layer and stricter governance for approvals.
I facilitated requirements workshops, documented process gaps, and prepared implementation-ready user stories.

### General Project Description
Program to harmonize onboarding and approval processes across business units with centralized workflow orchestration.
Roles: Business Analyst
Skills: Requirements Engineering, Workshop Facilitation, Process Analysis
```

### Output
```json
{
  "RationaleOriginalVersion": "Preserved all consultant-provided content except predefined excluded labels (none present). No Markdown structures required conversion beyond paragraph formatting. No forbidden tags were used.",
  "OriginalVersion": "<p>The project was initiated to standardize onboarding across five business units and to increase process transparency.</p><p>The target architecture introduced a centralized workflow layer and stricter governance for approvals.</p><p>I facilitated requirements workshops, documented process gaps, and prepared implementation-ready user stories.</p>",
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, dropped project-only context not tied to consultant contributions, and reduced overlap with project-description narrative. Applied adaptive sparse fallback when needed, avoided invented facts, and followed formatting rules (no forbidden tags, minimal HTML escaping only for `&amp;`, `&lt;`, and `&gt;`; normal UTF-8 apostrophes and quotation marks kept literal).",
  "BulletVersion": "<ul><li>Facilitation of requirements workshops across involved stakeholders</li><li>Documentation of process gaps as input for implementation planning</li><li>Preparation of implementation-ready user stories</li></ul>",
  "MixedVersion": "<p>The engagement contribution focused on translating stakeholder needs into implementation-ready delivery artifacts, and I converted workshop outcomes into implementation-ready user stories.</p><ul><li>Facilitated requirements workshops</li><li>Documented process gaps for delivery planning</li><li>Prepared implementation-ready user stories</li></ul>",
  "ParagraphVersion": "<p>The engagement contribution focused on requirements engineering and delivery preparation. I facilitated requirements workshops, documented process gaps, and prepared implementation-ready user stories for the next implementation steps.</p>"
}
```

## Example 9 (Adaptive Sparse Fallback and Empty Clean Outcome)

### Input
```markdown
### Consultant Description
Role: Integration Specialist
Skills: API Design, REST, Swagger

### General Project Description
Legacy modernization program with multiple integration streams.
```

### Output
```json
{
  "RationaleOriginalVersion": "Preserved consultant-provided role and skills content. No predefined excluded label categories were present. Markdown conversion produced paragraph output only.",
  "OriginalVersion": "<p>Role: Integration Specialist</p><p>Skills: API Design, REST, Swagger</p>",
  "RationaleCleanVersions": "Consultant-action evidence was too limited for concrete contribution statements. Adaptive sparse fallback was considered, but meaningful non-fabricated clean content was not feasible, so clean versions were returned empty.",
  "BulletVersion": "",
  "MixedVersion": "",
  "ParagraphVersion": ""
}
...
1.  **Input Usage**
    *   **`### Consultant Description`**:
        *   The **OriginalVersion** must be derived **exclusively** from this text, removing *only* items from the predefined exclusion list (Project Name/Title, Client Name, specific date labels, Industry, Functional Area – both label and value – and any resulting empty headers, as detailed in `Specifics 1.c`). **All other content (lists, tech mentions, paragraphs, table content, etc., not subject to exclusion) must be preserved after removing all Markdown formatting and reformatting into clean, valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) only. Preserve normal UTF-8 apostrophes and quotation marks as literal characters.**
        *   This segment may include headings, bullet points, tables, or Markdown formatting (like `**bold**`, which will be ignored).
    *   **`### General Project Description`**:
        *   The **BulletVersion**, **MixedVersion**, and **ParagraphVersion** can draw from both inputs, but consultant text remains primary and project text is used only to clarify/contextualize consultant actions.
        *   Avoid repeating full project-description storyline elements unless they are necessary to make a consultant action understandable.
...
2.  **Examples as Structural Guidance Only (If Provided Separately)**
    ...
    *   They would include rationale texts and the required HTML tags, reflecting the predefined exclusion rules, the no-bold rule, the table handling rule, and the minimal HTML escaping rule.
...
3.  **HTML Restrictions**
    ...
    *   **HTML Escaping**: Apply only minimal HTML escaping to text content within generated `<p>` and `<li>` tags in all four versions: `&amp;`, `&lt;`, and `&gt;` where required. Preserve normal UTF-8 apostrophes and quotation marks as literal characters, and do **not** emit quote/apostrophe entities such as `&#39;`, `&apos;`, `&#34;`, or `&quot;` unless they already appear in the source text and must be preserved literally.
...
6.  **Mandatory JSON Structure**
    Always return a single JSON object with **exactly** six keys:
...
7.  **Action-Oriented, Consultant-First Clean Versions (Bullet/Mixed/Paragraph)**
    *   **OriginalVersion**: Retains the consultant’s phrasing (including first-person). Includes **all** content from `### Consultant Description` not subject to predefined exclusions (`Specifics 1.c`), with **all Markdown removed** and structures reformatted (tables to `<p>Key: Value</p>`, lists to `<ul><li>`, headings to `<p>`). Output uses valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) only. Normal UTF-8 apostrophes and quotation marks stay literal. Only minimal grammar/spelling fixes. **No bold formatting.**
    *   **BulletVersion**: Transformed into a noun-based, impersonal bullet list focusing on consultant **actions**. Roles/skills/technologies integrated contextually, **not** as generic standalone points. Consultant description is primary; project description only clarifies consultant actions. Project-only context not tied to consultant contribution is dropped. No bold formatting. Output uses valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) only, normal UTF-8 apostrophes and quotation marks stay literal, and `<li>` text does not end with a period.
    *   **MixedVersion**: A short paragraph plus action bullets. Roles/skills/technologies integrated naturally and only when tied to consultant actions. Consultant description is primary; project description only clarifies consultant actions. Project-only context not tied to consultant contribution is dropped. The first sentence must not begin with `I` or `Ich`. **No bold formatting.** Output uses valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) only, normal UTF-8 apostrophes and quotation marks stay literal, and mixed-version `<li>` text does not end with a period.
    *   **ParagraphVersion**: A cohesive first-person narrative using `<p>` tags only and centered on consultant contributions. Roles/skills/technologies integrated naturally. Consultant description is primary; project description only clarifies consultant actions. Project-only context not tied to consultant contribution is dropped. The first sentence must not begin with `I` or `Ich`. **No bold formatting.** No bullet points. Output uses valid HTML with minimal HTML escaping (`&amp;`, `&lt;`, `&gt;`) only, and normal UTF-8 apostrophes and quotation marks stay literal.

8.  **Dealing with Technology Lists**
    *   If the `### Consultant Description` includes a line or paragraph listing technologies (e.g., “Technologies used: …”), and this label is not one of the predefined exclusions in `Specifics 1.c`, **preserve that entire line/paragraph** (within `<p>` tags, after removing any Markdown formatting like bold, and applying only minimal HTML escaping for `&`, `<`, and `>`) in the `OriginalVersion`.
...
12. **Complementarity with Project Description**: Engagement clean versions should not absorb project-summary content that is not consultant-specific. Keep project-only framing out of clean engagement outputs so it can be handled by project-description generation.
