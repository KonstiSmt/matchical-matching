...
- **Enriched Alternatives (Clean Versions Only)**: For the BulletVersion, MixedVersion, and ParagraphVersion, treat the `### Consultant Description` as the primary evidence source for what to include. Use the `### General Project Description` only to clarify, disambiguate, or lightly contextualize consultant actions that are already evidenced. Refrain from adding anything that is not present or reasonably implied in the inputs, and still exclude the predefined set of sensitive/redundant items. Roles and skills should be woven into descriptions of actions, not listed generically. **Project-level scope, goals, or outcomes must be omitted unless explicitly tied to the consultant’s actions.**
...
```json
{
  ...
  "RationaleCleanVersions": "Prioritized consultant-evidenced actions from the Consultant Description, used General Project Description details only to clarify those actions, and removed project-level scope not tied to consultant contributions. Reduced overlap with project-description narrative, integrated roles/skills/technologies contextually when evidenced, and avoided inventing new facts while following formatting rules (no forbidden tags, proper HTML entities).",
